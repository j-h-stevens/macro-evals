...
def build_document(trace: dict) -> str:
    ...
    for ev in events:
        ...
        # v010: latency-bucket tokens DISABLED (absence-only variant)
    ...
    expected = EXPECTED_AGENTS.get(trace["case_type"], set())              # B
    absent = sorted(expected - activated)
    if absent:
        parts.append("absent " + " ".join(f"absent:{a}" for a in absent))
