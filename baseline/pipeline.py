"""
Baseline pipeline: faithful reproduction of the OpenAI "Macro Evals for
Agentic Systems" cookbook method, applied to sim/data/traces.jsonl.

Faithfulness contract (see baseline/README.md for full citations):

  - Document construction uses agent handoffs, tool names, env_signals,
    state transitions, and run_outcome only. (source-claims §5.6)
  - Latency, expected-but-absent events, and timestamps are NOT used as
    features. These omissions are exactly what hypotheses H1, H2, H3
    predict the baseline will fail at; defeating those predictions is the
    Improver's job, not ours.
  - Underscore-prefixed fields (e.g., `_raw_price_usd`) are stripped.
  - random_state = 42 everywhere.
  - Per-failure-mode performance is NOT computed here. The Statistician
    consumes the raw cluster x failure_mode confusion matrix.
"""
...
def build_document(trace: dict) -> str:
    """Compact structured-summary document (source-claims §5.6.2).

    Includes: case_type, env_signals (intake), agent handoffs and tool
    calls in order, state transitions inferred from return statuses,
    terminal outcome. EXCLUDES: latency_ms (H1), absent events (H2),
    timestamps (H3), underscore-prefixed values (ground truth)."""
